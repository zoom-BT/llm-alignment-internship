...
- [~] Basic PyTorch mechanics (tensors, parameters, gradients, CUDA) — tensors and CUDA/device placement deeply covered (incl. two real device bugs fixed); "parameters" and "gradients" were *used* (via `SFTTrainer`) but never explicitly taught as their own concept — still open
...
- [~] Batches, epochs, learning rates, and optimizers (AdamW) — epochs and learning rate reasoned about directly (identifying the overfitting cause), but "batch" as a concept and AdamW's actual mechanics were never explicitly taught — still open
...
- [~] Gradient accumulation — configured in `config.yaml`/`run_sft`, but never explained mechanically (why simulating a larger batch this way works) — still open
- [~] Gradient checkpointing — same: configured, not yet explained mechanically — still open
- [x] Random seeds & strict reproducibility — deep dive
- [~] Model checkpoints saving/loading — done practically (`trainer.save_model`, reloading `results/final`), but never explained as a concept in its own right — still open
