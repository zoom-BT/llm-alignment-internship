...
- [x] manually inspect dataset examples and labels; — done, and it is what produced D8 and D9. Inspecting the PASS/FAIL examples showed they are compliant *helpful* answers rather than refusals, which invalidated the proposal's RR% metric; counting the `theme` field showed the corpus is 45% Honest against 25% Harmless. Reproducible in the code repo's `notebooks/01_inspection_donnees.ipynb`, committed with outputs — English examples first, since those are readable without knowing the target languages
- [x] document the source and license of each dataset; — `03_Experiments/Week5_Dataset_Description_Sheet.md`, six datasets. Two flags remain open and are recorded rather than closed: UbuntuGuard's licence rests on the paper's statement (no LICENSE file upstream, author contacted), and HealthBench-Africa's is unconfirmed. TukaBench's CC-BY-NC 4.0 is flagged as non-commercial
...
- [x] implement the evaluation metrics; — **redefined, then implemented.** RR% as specified measures nothing on UbuntuGuard, which is a guardian/classification benchmark (D8); the metric is now the guardian's macro F1, doubling as the stated precision of the compliance detector. Built in `src/metrics.py` (18 tests): both extractor modes and the gap between them, majority-class floor, per-language/per-theme breakdown with an underpowered flag, and McNemar for paired model comparison. Only *running* it against a model needs a GPU
...
- [x] a dataset description sheet; — `03_Experiments/Week5_Dataset_Description_Sheet.md`, six datasets each verified against primary sources rather than card prose
