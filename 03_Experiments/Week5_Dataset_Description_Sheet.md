...
- **Splits (per config):** `train`: 8, `test`: 809. Total rows per language, verified via the size API: `en` 817, `nso` 817, `am` 815, `yo` 815, `sw` 813, `ha` 799, `zu` 761. English matches the original TruthfulQA's 817 questions exactly; the African configs fall slightly short, so the translations are **not** a complete 1:1 rendering of all 817 in every language — do not assume perfectly parallel question sets across languages.
