...
**Retracted (2026-08-28): the tokenizer claim was wrong.** An earlier note here read that AfriqueQwen's 248,044-token vocabulary, "against roughly 151,000 for stock Qwen", showed its vocabulary extension measurably doing its job and supported H2's premise. Loading both tokenizers on Kaggle shows **`Qwen/Qwen3.5-4B-Base` reports the same 248,044**. Qwen3.5 ships that vocabulary natively; AfriqueQwen inherits it and extended nothing. The 151,000 figure was Qwen2.5's, carried over from memory rather than measured — exactly the kind of unchecked comparison this document exists to catch.

**What is true, and is worth more than the retracted claim:** the two backbones share a tokenizer. Tokenisation is therefore *perfectly controlled* between B3 and B4 — identical vocabulary, identical segmentation of the same text — so no part of any measured difference can be attributed to one model tokenising African languages more efficiently than the other. The single-variable design of H2 is stronger than assumed, not weaker. The measured ~4.1 characters per token stands as a property of Qwen3.5's tokenizer on this corpus, not as evidence for the CPT.
