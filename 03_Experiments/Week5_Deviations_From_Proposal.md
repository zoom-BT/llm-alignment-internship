...
**Counter-example found: Uhura-TruthfulQA is genuinely native-translated.** Confirmed via its dataset card: TruthfulQA was translated into 6 languages (Amharic, Hausa, Northern Sotho, Swahili, Yoruba, Zulu) by **professional human translators**, not an LLM pipeline. Not a safety/preference dataset (it's factuality QA, still D3/utility-side only), so it doesn't resolve D1/D2 directly — but it's evidence that native-quality resources do exist for at least some of our target languages, worth keeping in mind as a template for what to look for via the search prompt below.
